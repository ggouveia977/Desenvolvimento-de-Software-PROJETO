...
public class Aplicações {
    ...
    public void listarTaticas() {
        ...
        for (Tatica taticaExistentes : taticaController.listar()) {
    ...
    public void aplicarTatica(int opcaoEscolhida) { // pega a opção de tática escolhida...
        ...
        Tatica minhaTatica = taticaController.getTatica(opcaoEscolhida); // Novo objeto de Tatica que pega a opção
                                                                         // escolhida.
        ...
        if (jogadoresController.buscarPosicao("Ataque").size() >= minhaTatica.getNumDeJogadoresNoAtaque()) {
            ...
            for (int i = 0; i < minhaTatica.getNumDeJogadoresNoAtaque(); i++) { // Enquanto o numero de jogadores no
                                                                                // ataque for menor que o numero de
                                                                                // jogadores no ataque da tática ele
                                                                                // executa
                jogadoresAtaque.add(jogadoresController.buscarPosicao("Ataque").get(i)); // adiciona os jogadores no
                                                                                         // ataque.
            }
        }

        else {
            for (int i = 0; i < jogadoresController.buscarPosicao("Ataque").size(); i++) { // Enquanto o numero de
                                                                                           // jogadores no ataque for
                                                                                           // menor que o numero de
                                                                                           // jogadores no ataque da
                                                                                           // tática ele executa
        ...
        if ((jogadoresController.buscarPosicao("MeioCampo").size() - contJogadores) >= minhaTatica
                .getNumDeJogadoresNoMeioDeCampo()) {
            for (int i = contJogadores; i < minhaTatica.getNumDeJogadoresNoMeioDeCampo(); i++) { // Adiciona os
                                                                                                 // jogadores no meio
                                                                                                 // campo
                jogadoresMeioCampo.add(jogadoresController.buscarPosicao("MeioCampo").get(i));
            }
        } else {
            for (int i = contJogadores; i < jogadoresController.buscarPosicao("MeioCampo").size(); i++) { // Adiciona os
                                                                                                          // jogadores
                                                                                                          // no meio
                                                                                                          // campo
                jogadoresMeioCampo.add(jogadoresController.buscarPosicao("MeioCampo").get(i));
        ...
        if ((jogadoresController.buscarPosicao("Defesa").size() - contJogadores) >= minhaTatica
                .getNumDeJogadoresNaDefesa()) {
            for (int i = contJogadores; i < minhaTatica.getNumDeJogadoresNaDefesa(); i++) { // Adiciona os jogadores da
                                                                                            // defesa
                jogadoresDefesa.add(jogadoresController.buscarPosicao("Defesa").get(i));
            }
        } else {
            for (int i = contJogadores; i < jogadoresController.buscarPosicao("Defesa").size(); i++) { // Adiciona os
                                                                                                       // jogadores no
                                                                                                       // meio campo
                jogadoresDefesa.add(jogadoresController.buscarPosicao("Defesa").get(i));
            }
        ...
        taticaAplicadaController.adicionar(novaTaticaAplicada);

        this.taticaAplicadaSalva = novaTaticaAplicada;

    }

    // todo verificar se eu tenho 10 jogadores para aplicar uma tatica, caso não,
    // mandar uma mensagem de erro "você nao tem 10 jogadores! favor adicione 10"
    ...
    public void montarTatica() {
        ...
        if (ataque + defesa + meioCampo == 10) {
            ...
            taticaController.adicionar(tatica);
        } else {
